...
import { metodoBloques } from "@/data/metodo";
import SeoHead from "@/components/SeoHead";

export default function MetodoDetalle() {
  const { slug } = useParams<{ slug: string }>();
  const bloque = metodoBloques.find((b) => b.slug === slug);
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-serif font-bold">Bloque no encontrado</h1>
  ...
  const idx = metodoBloques.findIndex((b) => b.slug === slug);
  const prev = idx > 0 ? metodoBloques[idx - 1] : null;
  const next = idx < metodoBloques.length - 1 ? metodoBloques[idx + 1] : null;

  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden font-sans">
      <SeoHead
        title={`${bloque.title} — Método MECF`}
        description={bloque.tagline}
        canonical={`/metodo/${bloque.slug}`}
        type="article"
        jsonLd={{
          ...
          "description": bloque.tagline,
          "url": `https://metodomecf.com/metodo/${bloque.slug}`,
      ...
      <section className="pt-32 pb-16 relative">
        ...
        <div className="max-w-3xl mx-auto px-6 relative z-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="font-mono text-[10px] tracking-[0.3em] uppercase text-primary/50">MÉTODO MECF</span>
          </div>
      ...
      <section className="py-10 border-t border-white/8">
        <div className="max-w-3xl mx-auto px-6">
          <div className="font-mono text-sm text-muted-foreground p-8 border border-white/10 bg-white/[0.02] leading-relaxed">
            <p className="text-primary mb-2 text-[10px] tracking-widest uppercase">&gt; Siguiente fase</p>
            <p>{bloque.cierre}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-6 text-center space-y-5">
          <p className="font-mono text-xs tracking-widest uppercase text-primary/60">Sistema disponible</p>
          <h3 className="text-2xl md:text-3xl font-serif font-bold">
            Aplica el método a tu caso personal
          </h3>
          <p className="text-muted-foreground font-light">
            Calcula tus nodos cronológicos y recibe tu informe personalizado en PDF.
          </p>
          <Link
            href="/protocolo"
      ...
      <section className="py-8 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-6 flex justify-between items-center gap-4">
          {prev ? (
            <Link href={`/metodo/${prev.slug}`} className="group flex items-center gap-3 text-muted-foreground hover:text-primary transition-colors">
          ...
          {next ? (
            <Link href={`/metodo/${next.slug}`} className="group flex items-center gap-3 text-muted-foreground hover:text-primary transition-colors text-right">
              <div>
